// Package sm4 implements the SM4 block cipher per GB/T 32907-2016.
package sm4

import (
	"crypto/cipher"
	"encoding/binary"
	"strconv"
)

const (
	// BlockSize is the SM4 block size in bytes.
	BlockSize = 16
	// KeySize is the SM4 key size in bytes.
	KeySize = 16
)

// KeySizeError is returned when key size is invalid.
type KeySizeError int

func (k KeySizeError) Error() string {
	return "sm4: invalid key size " + strconv.Itoa(int(k))
}

// sm4Cipher is an instance of SM4 encryption.
type sm4Cipher struct {
	rk [32]uint32 // round keys
}

// NewCipher creates and returns a new cipher.Block.
// The key argument should be 16 bytes.
func NewCipher(key []byte) (cipher.Block, error) {
	if len(key) != KeySize {
		return nil, KeySizeError(len(key))
	}
	c := new(sm4Cipher)
	c.expandKey(key)
	return c, nil
}

func (c *sm4Cipher) BlockSize() int { return BlockSize }

func (c *sm4Cipher) Encrypt(dst, src []byte) {
	if len(src) < BlockSize {
		panic("sm4: input not full block")
	}
	if len(dst) < BlockSize {
		panic("sm4: output not full block")
	}
	encryptBlock(c.rk[:], dst, src)
}

func (c *sm4Cipher) Decrypt(dst, src []byte) {
	if len(src) < BlockSize {
		panic("sm4: input not full block")
	}
	if len(dst) < BlockSize {
		panic("sm4: output not full block")
	}
	decryptBlock(c.rk[:], dst, src)
}

func encryptBlock(rk []uint32, dst, src []byte) {
	x0 := binary.BigEndian.Uint32(src[0:4])
	x1 := binary.BigEndian.Uint32(src[4:8])
	x2 := binary.BigEndian.Uint32(src[8:12])
	x3 := binary.BigEndian.Uint32(src[12:16])

	for i := 0; i < 32; i += 4 {
		x0 ^= t(x1 ^ x2 ^ x3 ^ rk[i])
		x1 ^= t(x2 ^ x3 ^ x0 ^ rk[i+1])
		x2 ^= t(x3 ^ x0 ^ x1 ^ rk[i+2])
		x3 ^= t(x0 ^ x1 ^ x2 ^ rk[i+3])
	}

	binary.BigEndian.PutUint32(dst[0:4], x3)
	binary.BigEndian.PutUint32(dst[4:8], x2)
	binary.BigEndian.PutUint32(dst[8:12], x1)
	binary.BigEndian.PutUint32(dst[12:16], x0)
}

func decryptBlock(rk []uint32, dst, src []byte) {
	x0 := binary.BigEndian.Uint32(src[0:4])
	x1 := binary.BigEndian.Uint32(src[4:8])
	x2 := binary.BigEndian.Uint32(src[8:12])
	x3 := binary.BigEndian.Uint32(src[12:16])

	for i := 31; i >= 0; i -= 4 {
		x0 ^= t(x1 ^ x2 ^ x3 ^ rk[i])
		x1 ^= t(x2 ^ x3 ^ x0 ^ rk[i-1])
		x2 ^= t(x3 ^ x0 ^ x1 ^ rk[i-2])
		x3 ^= t(x0 ^ x1 ^ x2 ^ rk[i-3])
	}

	binary.BigEndian.PutUint32(dst[0:4], x3)
	binary.BigEndian.PutUint32(dst[4:8], x2)
	binary.BigEndian.PutUint32(dst[8:12], x1)
	binary.BigEndian.PutUint32(dst[12:16], x0)
}

// t is the round function T
func t(x uint32) uint32 {
	return l(tau(x))
}

// t' is the key expansion function T'
func tPrime(x uint32) uint32 {
	return lPrime(tau(x))
}

// tau is the nonlinear transformation (S-box applied to each byte)
func tau(x uint32) uint32 {
	return uint32(sbox[byte(x>>24)])<<24 |
		uint32(sbox[byte(x>>16)])<<16 |
		uint32(sbox[byte(x>>8)])<<8 |
		uint32(sbox[byte(x)])
}

// l is the linear transformation L
func l(x uint32) uint32 {
	return x ^ rotl(x, 2) ^ rotl(x, 10) ^ rotl(x, 18) ^ rotl(x, 24)
}

// l' is the linear transformation L' for key expansion
func lPrime(x uint32) uint32 {
	return x ^ rotl(x, 13) ^ rotl(x, 23)
}

func rotl(x uint32, n int) uint32 {
	return (x << n) | (x >> (32 - n))
}

// expandKey generates round keys from the master key
func (c *sm4Cipher) expandKey(key []byte) {
	mk0 := binary.BigEndian.Uint32(key[0:4])
	mk1 := binary.BigEndian.Uint32(key[4:8])
	mk2 := binary.BigEndian.Uint32(key[8:12])
	mk3 := binary.BigEndian.Uint32(key[12:16])

	k0 := mk0 ^ fk[0]
	k1 := mk1 ^ fk[1]
	k2 := mk2 ^ fk[2]
	k3 := mk3 ^ fk[3]

	for i := 0; i < 32; i += 4 {
		k0 ^= tPrime(k1 ^ k2 ^ k3 ^ ck[i])
		c.rk[i] = k0
		k1 ^= tPrime(k2 ^ k3 ^ k0 ^ ck[i+1])
		c.rk[i+1] = k1
		k2 ^= tPrime(k3 ^ k0 ^ k1 ^ ck[i+2])
		c.rk[i+2] = k2
		k3 ^= tPrime(k0 ^ k1 ^ k2 ^ ck[i+3])
		c.rk[i+3] = k3
	}
}

// System parameters FK
var fk = [4]uint32{
	0xA3B1BAC6, 0x56AA3350, 0x677D9197, 0xB27022DC,
}

// Fixed parameters CK
var ck = [32]uint32{
	0x00070E15, 0x1C232A31, 0x383F464D, 0x545B6269,
	0x70777E85, 0x8C939AA1, 0xA8AFB6BD, 0xC4CBD2D9,
	0xE0E7EEF5, 0xFC030A11, 0x181F262D, 0x343B4249,
	0x50575E65, 0x6C737A81, 0x888F969D, 0xA4ABB2B9,
	0xC0C7CED5, 0xDCE3EAF1, 0xF8FF060D, 0x141B2229,
	0x30373E45, 0x4C535A61, 0x686F767D, 0x848B9299,
	0xA0A7AEB5, 0xBCC3CAD1, 0xD8DFE6ED, 0xF4FB0209,
	0x10171E25, 0x2C333A41, 0x484F565D, 0x646B7279,
}

// S-box from GB/T 32907-2016
var sbox = [256]byte{
	,
}
